...
isDistinct l = case l of
  [] -> True
  x : xs -> notElem x xs && isDistinct xs
...
*** Gave up! Passed only 573 tests; 10000 discarded tests.
...
16% ordered - 7% length 0, 5% length 2, 2% length 1, 1% length 3, 1% length 5
...
-- [100,70,90,80,100,40,80,10,20,60,60]
...
-- [[],[],[],[],[1],[3,1,3,3],[3,2,3,2,1],[1,2],[1,3,2,1,3,3],[1,1,3,3,1,1,2,2,1,3,2,1,3],[1]]
...
-- [200,200,100,200,100,200,200,100,1,100,100]
...
-- arbitrary generates ints and genList1 generates lists
-- liftM2 will cons ints to a list and generate those one by one
...
Infinite loop since we will keep calling genList1
...
Too often, we are getting the empty list
